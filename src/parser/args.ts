...
export type ParseArgsResult<T = ParsedArgs> =
  | { ok: true; value: T }
  | { ok: false; error: AcliErrorResponse }

/**
 * Check if a value is a Zod-based ArgSchema
 */
...
/**
 * Check if args definition is Zod-based
 */
function isZodArgsDefinition(
  args: Record<string, ArgumentDefinition | ArgSchema>,
): args is ArgsDefinition {
  const values = Object.values(args)
  if (values.length === 0) return true
  return isArgSchema(values[0])
}

/**
 * Parse argument tokens into a structured object using Zod schemas
 */
export function parseArgsWithZod<T extends ArgsDefinition>(
  tokens: string[],
  argDefs: T,
  ...
  const positionalArgs = new Map<
    number,
    { name: string; schema: ArgSchema }
  >()
  ...
  if (!parseResult.success) {
    ...
    const hint = firstIssue.code === 'invalid_type' && (firstIssue as { received?: string }).received === 'undefined'
      ? `Provide --${path} <value>`
      : undefined
...
function isFlagSchema(schema: z.ZodType): boolean {
  // Check if it's a ZodDefault wrapping a boolean
 ...
 * Supports both legacy ArgumentDefinition and Zod-based ArgsDefinition
 */
...
): ParseArgsResult<ParsedArgs>
export function parseArgs(
  tokens: string[],
  argDefs: Record<string, ArgumentDefinition | ArgSchema>,
): ParseArgsResult {
  // Check if it's a Zod-based definition
  if (isZodArgsDefinition(argDefs)) {
    return parseArgsWithZod(tokens, argDefs)
  }

  // Cast to legacy type after zod check
  const legacyArgDefs = argDefs as Record<string, ArgumentDefinition>

  // Legacy parsing logic
  const result: ParsedArgs = {}

  // Set defaults
  for (const [name, def] of Object.entries(legacyArgDefs)) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (token.startsWith('--')) {
      ...
      const [key, inlineValue] = token.slice(2).split('=')
      const argName = key.replace(/-/g, '_')
      const def = findArgDef(legacyArgDefs, key)
      ...
      const def = findArgDefByShort(legacyArgDefs, key)
